...
class SpotifySearch:
    def __init__(self, query, query_type, save_dir=''):
        ...
        self.console.info(
            'Searching for <LC>{0}<W> (<LC>{1}<W>)'.format(query, query_type))
        self.begin_search()

    def begin_search(self):
        results = self.SPClient.search(
            self.query, limit=15, type=self.query_type)
        index = 1
        creator_type = 'owner' if (
            self.query_type == 'playlist') else 'artists'
        if (len(results[self.query_type + 's']['items']) > 0):
            self.console.info('<M>{0:>2} {1:>35} {2:>45} {3:>25}'.format(
                'no.', creator_type, self.query_type +
                ' name', 'release date'), True)
            for data in results[self.query_type + 's']['items']:
                ...
                self.console.info('#{0:>02d} {1:>35} {2:>45} {3:>25}'.format(
                    index,
                    artist_name[:32].strip() +
                    '...' if len(artist_name) > 35 else artist_name.strip(),
                    item.name[:32].strip() +
                    '...' if len(item.name) > 35 else item.name.strip(),
                    item.release_date.split(
                        '-')[0] if ('release_date' in item) else '-'
                ), True)
                index += 1
            selected_number = int(
                input('\nSelect search item number:\n> #')) - 1
            if (selected_number >= 0 and selected_number <= index):
                selected_item = objectify(
                    results[self.query_type + 's']['items'][selected_number])
                artist_name = ''
                if (creator_type == 'owner'):
                ...
                self.console.success(
                    'Selected {0} is <G>{1} - {2}<W>, uri id is {3}'.format(
                        self.query_type,
                        artist_name,
                        selected_item.name,
                        uri_id))
                if (self.query_type == 'album'):
                    ...
                    path_name = os.path.join(
                        self.save_dir, validate.file_name(
                            selected_item.name + '.txt'))
                    if (self.save_dir.endswith('.txt')):
                        path_name = self.save_dir
                    with open(path_name, 'a+') as stream:
                        stream.write(
...
class LoadPlaylist:
    def __init__(self, username, playlist_id, save_path=''):
        ...
        self.console.success(
            'finished loading playlist, {0} songs were found'.format(
                len(self.tracks)))
        self.dump_file()

    def request_playlist_info(self):
        playlist = objectify(self.SPClient.user_playlist(
        ...
        self.console.success('Playlist is <G>{0}<W> by <G>{1}'.format(
            self.metadata.name, self.metadata.owner))
    ...
    def dump_file(self):
        ...
        contents = (
            '#playlist\n@name {0}\n@description {1}' +
            '\n@owner {2}\n@cover {3}\n\n').format(
                self.metadata.name, self.metadata.desc,
                self.metadata.owner, self.metadata.cover)
...
class LoadAlbum:
    def __init__(self, album_id, save_path=''):
        ...
        self.console.info(
            'Gathering album data (<LC>ID_{0}<W>)'.format(album_id))
    ...
    def request_album_info(self):
        ...
        self.console.success('Album <G>{0}<W> by <G>{1}'.format(
            self.metadata.name, self.metadata.artist))
        self.console.success(
            '<G>{0}<W> tracks were found'.format(album.tracks.total))
    ...
    def dump_file(self):
        file_path = os.path.join(self.save_path, self.metadata.name + '.txt')
        self.console.info(
            'Writing dump file to directory <LC>{0}'.format(file_path))

        contents = '#album\n@name {0}\n@artist {1}\n@cover {2}\n\n'.format(
            self.metadata.name, self.metadata.artist, self.metadata.cover)
        for track in self.tracks:
            contents += '{0}\n'.format(track.link, track.name)
