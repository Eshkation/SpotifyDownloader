...
""")
	...
	def get_given_params(self):
		...
		if (params.folder):
			...
			self.console.info('Save directory set to <LC>{0}'.format(self.save_directory), True)

		if (params.nofilename):
			self.include_dump_file_name = False
			self.console.info('Save directory will no include dump file name', True)

		if (params.includealbumname):
			self.include_album_name = True
			self.console.info('Save directory will include track\'s album name', True)
		...
		if (params.source):
			self.console.info('Given source is <LC>{0}<W>, identifying url type'.format(params.source))
			self.identify_given_source(params.source)

		elif (params.download):
			self.console.info('Downloading tracks from <LC>{0}'.format(params.download))
	...
	def identify_search(self, query):
		if (':' in query):
			...
			if (search_type in ['album', 'playlist', 'track']):
				...
				return
		self.console.error('Search must be (album:|track:|playlist:)name, e.g: ==init.py -l album:m83 - junk==')
	...
	def prepare_download(self, dump_file):
		...
		while content_lines:
			...
			if (line.startswith('http')):
				...
				if (track_id):
					...
					self.console.success('Found track: <G>{0}<W> by <G>{1}<W>'.format(track.metadata.name, track.metadata.artist))
					...
					if (os.path.exists(file_path_name.replace('%(ext)s', 'mp3'))):
						self.console.warning('Audio file already exists, skipping download')
					else:
						youtube_video = SearchSong(track, self.download_low_score)
						if (youtube_video.metadata):
							download = DownloadVideo(track, youtube_video, final_save_dir)
							if (not download.SUCCESS):
								self.console.warning('Could not process track <R>ID_{0}'.format(track_id))
		...
		self.console.info('Total process time == <LC>{0:>08}'.format(str(datetime.timedelta(seconds = total_seconds))))
	...
	def identify_given_source(self, source_url):
		...
		if (private_playlist):
			...
			self.console.error('Could not identify source url: =={0}=='.format(source_url))
